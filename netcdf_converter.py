"""
This module provides functionality to convert raw sonar files into netCDF format using Echopype
...
- Converting raw files into netCDF format, with support for distributed computation.
...
def find_raw_files(inputs: Iterable[str]) -> List[str]:
    ...
    for inp in inputs:
        ...
        # Expand glob patterns if present; otherwise treat as a single path
        candidates = glob.glob(inp) if any(ch in inp for ch in "*?[]") else [inp]
        for c in candidates:
            if os.path.isfile(c):
                seen.add(c)
            else:
...
def convert(inputs: Iterable[str],
            ...
            use_swap: bool = True):
    """
    Converts a list of `.raw` files into a specified format and saves them in a given or default directory.
    ...
    save_dir: str | None, optional
        ...
        `echodata_nc` in the current working directory.
    ...
    if save_dir is None:
        ...
        base_dir = os.getcwd()
        save_dir = os.path.abspath(os.path.join(base_dir, "echodata_nc"))
    else:
    ...
    # Parse `.raw` file and save to netCDF format
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert EK80 .raw files to netCDF using Echopype and Dask.")
    parser.add_argument(
        "inputs",
        nargs="+",
    ...
    parser.add_argument(
        ...
        dest="save_dir",
        default=None,
        help="Output directory to store netCDF files (default: ./echodata_nc under input dir)",
    )
    parser.add_argument(
        "--sonar-model",
        default="ek80",
